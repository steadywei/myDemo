...
public class Tree2ExcelUtil {
     ...
     * @return
     */
    public static boolean tree2Excel(List<Map> treeList, String filePath) {
        return tree2Excel(treeList, filePath, null, null);
    }
     ...
     * @return
     */
    public static boolean tree2Excel(List<Map> treeList, String filePath, String lableName, String childrenName) {
        if (isBlank(filePath)) {
            System.err.println("文件名称不能为空");
            return false;
        }
        try {
            doSame(treeList, lableName, childrenName);
            createExcel(filePath, treeList);
            return true;
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public static Workbook tree2Worbook(List<Map> treeList, String fileSuf) {
        return tree2Worbook(treeList, fileSuf, null, null);
    }
     ...
     * @return
     */
    public static Workbook tree2Worbook(List<Map> treeList, String fileSuf, String lableName, String childrenName) {
        if (isBlank(fileSuf)) {
        ...
        try {
            doSame(treeList, lableName, childrenName);
            return procesData(treeList, fileSuf);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private static void doSame(List<Map> treeList, String lableName, String childrenName) {
     ...
     * @throws IOException
     */
    private static void createExcel(String filePath, List<Map> treeList) throws IOException {
        File file = new File(filePath);
        boolean bfile = file.createNewFile();
        // 复制模板到新文件
        if (bfile) {
            Workbook wk = procesData(treeList, filePath);
            if (wk != null) {
     ...
     * @return 工作表对象
     */
    private static Workbook procesData(List<Map> treeList, String fileName) {
        ...
        for (Map tree : treeList) {//创建一个sheet页
            Object name = tree.get("name");
            Sheet sheet = wk.createSheet(String.valueOf(name));
